...
def colorLog(line:str,type:int):
  ...
  if type==1 or type==3:
    ...
    if type==3:
      ...
      output=re.sub("(\d{5,})",r"<span class='int'>\1</span>",output)#int
  elif type==2:
    line=re.sub("\[.+?m","",line)
    line=re.sub("","",line)
    ...
    line=re.sub("\[([A-Za-z0-9\s-]+?)\]",r"[<span class='plugins \1'>\1</span>]",line)  #plugins
